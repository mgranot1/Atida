...
def insert_document():
    ...
    try:
        ...
        return jsonify({'error': 'Invalid date of birth (date_of_birth should be a date string in the format YYYY-MM-DD)'}), 400
    ...
    if data['corona_virus_details'].get('vaccines_details'):
        if not isinstance(data['corona_virus_details']['vaccines_details'], list):
            return jsonify({'error': 'Invalid data type for vaccine details'}), 400

        if len(data['corona_virus_details']['vaccines_details']) > 4:
            return jsonify({'error': 'Invalid amount of vaccines'}), 400

        for vaccine_details in data['corona_virus_details']['vaccines_details']:
            if not isinstance(vaccine_details, dict):
                return jsonify({'error': 'Invalid data type for vaccine details'}), 400
            if not vaccine_details.get('date_received') or not vaccine_details.get('manufacturer'):
                return jsonify({'error': 'Missing required subfields in vaccine details'}), 400
            # Check that the date_received field is a valid date string in the format YYYY-MM-DD
            print(vaccine_details['date_received'])
            try:
                date_received = datetime.datetime.strptime(vaccine_details['date_received'], '%Y-%m-%d')
            except ValueError:
                return jsonify(
                    {
                        'error': 'Invalid date received (date_received should be a date string in the format YYYY-MM-DD)'}), 400

            if vaccine_details['manufacturer'] not in vaccine_manufacturer:
                return jsonify({'error': 'Vaccine manufacturer '+vaccine_details['manufacturer']+' is not exist'}), 400

        # Sort the vaccines by date received
        data['corona_virus_details']['vaccines_details'].sort(key=lambda x: list(x.values())[0])
...
def get_summary():
    ...
    for record in data:
        if not record['corona_virus_details'].get('vaccines_details') or not len(record['corona_virus_details']['vaccines_details']):
            amount_not_vaccinated +=1
        # Parse the dates of illness and recovery from the record
        if record['corona_virus_details'].get('positive_result') and record['corona_virus_details']['positive_result'].get('date_received'):
            date_received = datetime.datetime.strptime(
